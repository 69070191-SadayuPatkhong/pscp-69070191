...
```text
input :
จํานวนเต็ม X และ Y ในบรรทัดเดียว
output :
จํานวนครั้งกระโดดขั้นตํ่าที่ทําให้ระยะรวม ≥ Y หรือ -1 ถ้าไปไม่ถึง โดยที่การกระโดดแต่ละรอบ จะลดไปที่ละ 2
constraints:
input เป็นจำนวนเต็มแต่อยู่ในบรรทัดเดียวกัน ต้องแยกก่อน
```
...
```text
step 1: อ่านค่าจำนวนเต็ท X, Y
step 2: วนซ้ำโดยที่ X > 0 นับจำนวนรวมแล้วเพิ่ม จำนวนที่นับแล้วที่ละ 1
step 3: ถ้าจำนวนรวม มากกว่าหรือเท่ากับ Y ให้ พิมพ์ค่าทรานับได้แล้วหยุด หากไม่ให้ลด X ลง 2
step 4: พิ่มผลนับ
```
