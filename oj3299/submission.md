...
```text
input :
รับค่า จำนวนเต็ม 2 ค่า L และ N

Output :
หมายเลขของแถบที่ช่องดอกไม้ลำดับที่ N ตก

Constraints :
input มี 2 ตัวในบรรทัดเดียวต้องใช้ split
```
...
```text
Step 1: อ่านค่า L และ N
Step 2: กำหนด stripe = 1 และ total = 0
Step 3: คำนวณจำนวนช่องในแถบที่ stripe ด้วยสูตร L * (((k - 1) * L + 1) + (k * L)) // 2
Step 4: บวกจำนวนช่องเข้าใน total
Step 5: ถ้า N น้อยกว่า total ให้พิมพ์ stripe แล้วจบการทำงาน
Step 6: ถ้ายังไม่ถึง ให้บวก stripe เพิ่มขึ้น 1 แล้วกลับไปทำ Step 3
```
