...
```text
input : รับอันตราการขยายของหมึกต่อวินาที และจำนวนบ้านที่โดน
แล้วรับพิกัดของบ้านตามจำนวนบ้านที่โดน
output : แสดงเวลา (วินาที) ที่บ้านแต่ละพิกัดจะโดนท้วม
constraints : input จำนวนเต็ม S (1 <= S <= 100000)  และบรรทัดที่ i + 1 (เมื่อ 1 <= i <= N) ระบุจำนวนเต็ม Xi และ Yi (0 <= Xi. Yi <= 10000)
```
...
```text
step 1 : ไปดูสูตร พื้นที่ของ วงกลม
step 2 : อ่านค่า อันตราท้วมและ จำนวนบ้านที่โดน
step 3 : ใช้ค่า จำนวนบ้านที่โดน loop รับค่าพิกัดแต่ละบ้าน
step 4 : คำนวนระยะทางจาก 0, 0 ไปยังพิกัดบ้าน 
step 5 : คำนวณ area ณ ตอนนั้น
step 6 : คำนวณ เวลา โดนการเอา area หารด้วย อันตราท้วม
step 7 : ปัดเวลาขึ้น แล้วพิมค์ค่าเวลา
```
...
```text
เหมื่อนเดิม เพราะ ข้อนี้ไม่เกิด edge case เช่นเลขติดลบแน่นอน และพอส่งแล้วถูกต้องจึงไม่ได้เปลี่ยนอะไร
