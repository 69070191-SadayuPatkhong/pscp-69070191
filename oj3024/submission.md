...
```text
input : คะแนนรวมของ 3 คน และ คะแนนสูงสุดใน 3 คนนี้
output : แสดงข้อความ Surprising หากมีโอกาสที่คะแนนต่ำสุดที่เป็นไปได้จะต่างจากคะแนนสูงสุดเกิน 2 คะแนนแสดงข้อความ Not surprising หากไม่มีโอกาสเกิดขึ้น
Constraints: score รวมทั้ง 3 คน ต้องอยู่ในช่วง 0 - 30 และ คะแนนสูงสุดเป็นได้แค่ 0 - 10
```
...
```text
Step 1: อ่านคะแนนรวม และคะแนนสูงสุด
Step 2: คำนวณคะแนนต่ำสุดที่เป็นไปได้ min_score = total - 2 * max_score
Step 3: ถ้า min_score < 0 ให้ปรับ min_score = 0
Step 4: ตรวจสอบว่าข้อมูลถูกต้อง 0 <= max_score <= 10 และ max_score <= total <= 3 * max_score
Step 5: ถ้าข้อมูลถูกต้อง และ max_score - min_score > 2 ให้พิมพ์ "Surprising"
Step 6: ถ้าไม่ตรงตามเงื่อนไขให้พิมพ์ "Not surprising"
```
...
```text
เหมือนเดิมเพราะ ทำแล้วถูกต้องเลยไม่ได้เปลี่ยน (จริงๆเพราะผมทำโจทย์ก่อน)
```
...
```text
คะแนน มากสุดแค่ 5 แต่ มีคะแนนรวมถึง 12
```
...
```text
คะแนนเยอะมากแต่ คะแนนรวม 3 คน แค่ 10 เอง
```
