...
```text
โจทย์ให้ นำก้อนอิฐ อันใหญ่มาใช้ให้พอก่อน แล้ว นำอันเล็กมาใช้ให้ครบ หากไม่ได้ให้ตอบ -1
Input: รับค่าจำนวนเต็ม 3 ตัว
Output: จำนวนอิฐขนานเล็กที่ใช้ หากทำไม่ได้ให้ตอบ -1
Constraints: จำนวนอิฐไม่ควรเป็นลบ แต่เป็น 0 ได้
```
...
```text
Step 1: รับค่าตัวแปร a b goal
Step 2: หาจำนวนอิฐใหญ่ที่ต้องใช้ (ไม่เกิน b ก้อน และไม่เกินความยาว goal)
Step 3: คำนวณความยาวที่เหลือที่ต้องใช้อิฐเล็ก
Step 4: ตรวจสอบว่ารวมแล้วถึง goal รึป่าวหากไม่ให้ตอบ -1 หากใช้ให้ตอบจำนวนอิฐเล็กที่ใช้
```
...
```text
เหมือนแผนแรก แค่ตอนแรกคิดเลขผิดนิดหน่อยเลย เปลี่ยน แค่ code แต่การคิดยังเหมือนเดิม
```
...
```text
0 เพราะมันต้องสร้างได้โดยใช้ 0 ก้อน
```
