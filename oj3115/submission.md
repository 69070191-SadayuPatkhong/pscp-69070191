...
Output:
จำนวนร้านค้าที่เปิดอยู่ ณ เวลาที่ทำการเช็กแต่ละจุด (แสดงผลแยกด้วยช่องว่าง)

Constraints:
ทุกตัวเป็นจำนวนเต็ม และเวลามีค่าแค่ 0 ถึง 1440 (เท่ากับ 24 ชั่วโมงของ 1 วัน)
```
...
```text
Step 1: รับค่าจำนวนร้านค้าและ time จากบรรทัดแรก
Step 2: สร้างลิสต์ store_time เพื่อเก็บช่วงเวลาเปิด-ปิด (s, e) ของแต่ละร้านจำนวน store_amount บรรทัด
Step 3: อ่านรายการเวลาที่ต้องการตรวจสอบทั้งหมดเข้ามาเก็บในลิสต์ time_checks
Step 4: สร้างลิสต์ result ขนาดเท่ากับ time_checks โดยเริ่มต้นให้ทุกตำแหน่งมีค่าเป็น 0
Step 5: วนลูปตรวจสอบทีละเวลาใน time_checks เปรียบเทียบกับช่วงเวลา [s, e] ของทุกร้านใน store_time
Step 6: ถ้าเวลาเช็กนั้นอยู่ระหว่างเวลาเปิดและปิด (s <= check_val < e) ให้เพิ่มค่าใน result ของตำแหน่งนั้นขึ้น 1
Step 7: พิมพ์ค่าทั้งหมดใน result ออกมาทางหน้าจอโดยแยกด้วยช่องว่าง
```
...
```text
ใช้แผ่นเดิมเพราะ array ใช้ได้ดีใน โจทย์ข้อนี้
```
...
```text
เวลาที่ตรงกับจุดเริ่มต้นและจุดสิ้นสุดของช่วงพอดี
```
