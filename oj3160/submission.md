...
```text
input: เลขจำนวนเต็ม 2 ตัว
ตัวแรกเป็นตัวเริ่มต้นและตัวที่สองเป็นตัวจบของ range

output: หากมี prime number ใน range ให้พิมพ์ตัวเลขเหล่านั้น
พิมพ์จำนวน prime number ที่มี

constraints: 
input มี 2 ตัวแปรแต่อยู่ในบรรทัดเดียวกันต้อง split ออกกันก่อน
```
...
```text
step 1: import math และทำ function หา prime
step 2: รับค่าเริ่มและค่าจบ
step 3: สร้าง array ด้วยค่าเริ่มและค่าจบโดยผ่าน if prime
step 4: พิมพ์ array ที่ unpack หากมีค่าสักตัว
step 5: พิมพ์ จำนวน prime ที่มีใน array
```
